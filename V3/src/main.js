...
let particlesMaterial = new THREE.PointsMaterial({
  ...
  color: 0x6657B4, // Brand Blue/Indigo
  transparent: true,
    ...
    steps.forEach((step, i) => {
      ...
      if (node) {
        ...
        if (i === index) {
          ...
          gsap.to(num, { borderColor: '#6657B4', color: '#6657B4', scale: 1.1, duration: 0.3 })
          if (label) label.style.color = '#fff'
          if (status) {
            status.style.color = '#6657B4'
          ...
          if (card) {
            gsap.to(card, { 
              ...
              scale: 1.02, 
              borderColor: '#6657B4', 
              boxShadow: '0 30px 60px -15px rgba(102, 87, 180, 0.15)',
              duration: 0.4 
            })
          }
...
if (industryRows.length > 0 && industryContent) {
  const activateRow = (index) => {
    industryRows.forEach((r, i) => {
      ...
      if (i === index) {
        r.style.borderColor = '#6657B4'
        if (titleSpan) titleSpan.style.color = '#6657B4'
